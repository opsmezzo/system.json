...
exports.cycles = function (systems) {
  ...
  function addCircular(system) {
    ...
    deps.forEach(function (name) {
      ...
      if (!ref) {
        return addCircular(system.remoteDependencies[name]);
      }

      if ((match = refPattern.exec(ref))) {
        pair = match[1];
        circular[pair]        = circular[pair]        || [];
        circular[system.name] = circular[system.name] || [];

        circular[pair].push(system.name);
        circular[system.name].push(pair);
      }
    });
  }

  //
  // Attempt to build the set of circular dependencies
  // by looking up any "$ref" keys in the remoteDependencies
