...
exports.cycles = function (systems) {
  ...
  // remoteDependencies.
  //
  function iterateDependencies(system, iter) {
  ...
  names.forEach(function (name) {
    iterateDependencies(decycle[name], function addRef(system, name) {
      var ref = system.remoteDependencies[name]['$ref'],
          refName,
          match;

      if (!ref) {
        return iterateDependencies(system.remoteDependencies[name], addRef);
      }

      if ((match = refPattern.exec(ref))) {
        refName = match[1];
        refs[system.name] = refs[system.name] || {
          list: [],
          remoteDependencies: system.remoteDependencies
            && Object.keys(system.remoteDependencies)
        };

        refs[system.name].list.push(refName);
      }
    });
  });

  //
  // Build the set of circular dependencies by cross referencing
  // any remoteDependencies with any existing "$ref" keys
  // for any decycled systems.
  //
  names.forEach(function (name) {
    iterateDependencies(decycle[name], function checkCycle(system, name) {
      if (refs[name] && ~refs[name].list.indexOf(system.name)) {
        addCycle(system.name, name);
      }

      iterateDependencies(system.remoteDependencies[name], checkCycle);
    });
  });
