    ...
    async.map(options.clusters, client.servers.bind(client), function (err, configs) {
      ...
      configs.forEach(function (config) {
        Object.keys(config).forEach(function (system) {
          ...
          Array.prototype.push.apply(result[system], config[system]);
        });
      });

      //
      // Sort servers depending on how many clusters they appear in.
      // This way {{ servers.{{ system }}.0 }} might actually be the best
      // choice.
      //
      // Remark: this code is suboptimal (this algorithm could be implemented
      // with lower complexity), but this should never become a performance
      // concern (HUGE cluster would be needed to demonstrate this in the real
      // world).
      //
      // Another take on this is that by the time it becomes an actual problem,
      // we'll all be filthy rich and not really bothered anymore.
      //
      Object.keys(result).forEach(function (system) {
        var occurences = [];

        result[system].forEach(function (server) {
          var occurence = occurences.filter(function (occ) {
            return deepEqual(occ.server, server);
          })[0];

          return occurence
            ? ++occurence.count
            : occurences.push({ server: server, count: 1 });
        });

        result[system] = occurences.sort(function (a, b) {
          return b.count - a.count;
        }).map(function (occurence) {
          return occurence.server;
        });
      });
